...
function [GC_matrix, F_matrix, p_matrix] = compute_granger_causality(X, maxLag)
...
%
    ...
    try
        % For each pair (i → j):
        % Full vs reduced models are compared
        % Fij is proportional to how much removing i worsens prediction of j
        F_matrix = zeros(N,N);
        p_matrix = zeros(N,N);
